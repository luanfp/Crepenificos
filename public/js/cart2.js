...
// Calculate Payback On KeyPress Input Value Received By Client
$( "#amountReceivedByClient" ).on('keyup',function(e) {

  if(e.which == 13) {
    console.log("Apertou tecla ENTER ");
    if($(".processOrder").attr('disabled')){
      console.log("Digite o valor recebido.");
    }else{
      $(".processOrder").click();
    }
  }else{
    var received = $("#amountReceivedByClient").val();
    receivedArray = received.split(",");
    received = receivedArray[0];
    var purchaseTotal = parseInt($(".total .value").text().replace("R$ ", ""));
    var paybacktoClient = received-purchaseTotal;
    console.log("Digitou Troco: "+$("#amountReceivedByClient").val());
    $(".paybackValue").text(paybacktoClient+".00");
    if(paybacktoClient<0){
      $(".processOrder").prop("disabled",true);
    }else{
...
$('.go').click(function () {
    ...
    $("#dialog-payback").dialog({
                 open: function(event, ui){
                   $(".processOrder").prop("disabled",true);
                 },
                 title: "Finalizar a Venda:",
                 resizable: false,
                 height: 550,
                 width: 650,
                 modal: true,
